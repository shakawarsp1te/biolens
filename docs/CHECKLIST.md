...
- [x] Competitive research + infrastructure plan, "The BioLens Playbook" (Aug 25, 2026) — a researched read on fintech (Robinhood, Public, Webull), biotech-catalyst (CatalystAlert, BiopharmaWatch, Biostock), and habit-driven health apps (MyFitnessPal, Whoop), mapping what each does well against BioLens's own non-negotiables (no price predictions, no social layer, no sentiment scores, no trading) and proposing a phased feature/infrastructure roadmap. Published as an artifact; not itself a code change, but the direct source of the next entry.
- [x] Cash runway on every profile, Phase A of the Playbook (Aug 25, 2026) — the first shipped item from the roadmap above. `api/app/services/sec_edgar.py` (a free, official, no-key client for SEC EDGAR's ticker->CIK map and XBRL company-facts API — Tier 1 per PLAN.md §3.5, same tier as ClinicalTrials.gov) feeds `api/app/services/financial_health.py`, a deterministic calculation (never LLM-touched) of cash-on-hand and the most recent quarter's operating burn, exposed as `GET /market/financial-health/{ticker}`. Getting the burn figure right required a real fix mid-build: GAAP only requires a 10-Q's cash-flow statement to cover year-to-date, not the discrete quarter (unlike revenue/EPS, which are usually tagged both ways), so the naive "find a ~90-day 10-Q entry" approach silently fell back to a stale prior quarter for any company past its fiscal Q1 — caught by checking real live data (Cardiff Oncology's cash-as-of date and burn-period-end didn't match), fixed by deriving the discrete quarter as the difference between two consecutive year-to-date filings sharing the same fiscal-year start, with an explicitly-flagged averaged estimate as the last-resort fallback. Mobile: `FinancialHealthCard.tsx`, fetched independently on the company profile (same self-contained-fetch pattern as `StockQuoteCard`), placed right before Thesis Map since runway risk is exactly what "what could go wrong" already discusses. 30 new backend tests (calculation, client, router). Verified live against all 10 real seed companies' actual SEC filings — correct, internally-consistent cash/burn dates across the board, including Janux Therapeutics' cash-flow-positive quarter correctly reporting no runway with an explanation instead of a nonsensical negative number.
- [x] Catalyst calendar (trial-readout events), continuing the Playbook (Aug 25, 2026) — `GET /companies/{id}/catalysts` (`api/app/services/catalysts.py`) surfaces every upcoming trial-completion date across a company's real pipeline, derived purely from ClinicalTrials.gov's own `primaryCompletionDateStruct`/`completionDateStruct` on each trial already listed in that company's `pipeline[].trialIds` — each date typed `ESTIMATED` or `ACTUAL` by the trial's own sponsor, no LLM and no invented PDUFA guess anywhere in the pipeline (`app/services/clinicaltrials.py`'s `parse_study_summary` extended to expose these two fields, verified against a live NCT lookup before writing the parser). A recently-`ACTUAL` date (within 90 days) is still shown as a fresh readout; an old one or a past `ESTIMATED` one is filtered out — this is a forward-looking calendar, not a trial archive. A true PDUFA-decision-window event type (derived from a company's own disclosed NDA/BLA submission date) is a deliberate fast-follow, not built here — extracting a disclosed submission date out of filing/press-release text needs an LLM-assisted pipeline closer to `discovery.py`, not this deterministic CT.gov-only pass. Mobile: `CatalystCalendarCard.tsx`, same independent-fetch-on-mount pattern as `FinancialHealthCard`, placed between Pipeline and Cash & Runway. 9 new service tests + 3 new router tests. Verified live against all 10 real seed companies' actual CT.gov trial records — correct dates, correct month/day-precision handling (CT.gov sometimes only discloses "YYYY-MM"), and Cardiff Oncology's Nov 2026 catalyst sitting directly above its 2.3-month cash runway is exactly the connected-risk picture this feature was meant to surface.
- [x] Daily scan job + three live-caught bugs (Sep 25, 2026) — `api/scripts/run_scan.py` scheduled by `api/scripts/install_daily_scan.sh` (launchd, 8:00 daily, discovery on Mondays). Verified by triggering it under launchd (exit 0, 12 companies). Fixed PubMed 429s (retry with backoff), an exact-match-only large-pharma denylist that let two J&J entities through, and esearch results cached forever in-process.
- [x] Paper impact calls + track record (Sep 25, 2026) — PLAN.md §3 rule 10. `api/app/services/paper_impact.py` makes a validated, repair-looped call per new paper from its abstract; `signal_outcomes.py` scores calls against XBI-relative returns at 1/5/20 trading days (baseline = first close a reader could act on, after-close detections roll to the next day); `GET /signals/track-record`; Ask BioLens declines personal-portfolio questions. App: `ImpactCall.tsx` (expandable call on every paper row), `ResearchDisclaimer.tsx`, `app/track-record.tsx`. Verified live: 16 real calls on existing papers, read by hand for quality (reviews correctly rated "unlikely to matter", a mis-matched pesticide paper correctly flagged irrelevant). Surfaced and fixed a real bug in the process: PubMed abstracts were silently truncated at the first inline `<i>`/`<sup>` tag, which also affected Search and Ask BioLens.
